...
\begin{definition}\label{def:directionField}
  A \dfn{slope field}
  %, also called a \dfn{direction field}, 
...
% As we will see, the combination of direction fields and integral curves gives useful insights into the behavior of the solutions of the differential equation even if we cannot obtain exact solutions.
...
% used to plot solution curves of \eqref{eq:1.3.1} in a rectangular region $R$ if $f$ is continuous on $R$. 
...
%\subsection{Slope Fields vs Direction Fields}
%In Definition \ref{def:directionField} we stated that the terms slope fields and direction fields are often used interchangeably.  However, there is a subtle difference that we address here.  When dealing with an equation of the form $y'=f(x,y)$, especially when $x$ does not represent time, we are typically interested in the geometric shape of a solution rather than the ''flow" of a solution.  In such cases slopes are sufficient to help us identify the shape.  
...
\begin{center}  
\geogebra{a9n7zqdv}{800}{600}  
\end{center}

A slope field where the line segments are given ``arrow heads'' in this manner is called a \dfn{direction field}.  We will make use of direction fields later in the book. 
