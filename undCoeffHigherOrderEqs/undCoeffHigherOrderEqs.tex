...
$$
In \href{https://xerxes.ximera.org/differentialequations/main/higherOrderConstCoeff/higherOrderConstCoeff}{Trench 9.2} we learned how to find $y_c$. Here
we
...
method that we used in Trench \href{https://xerxes.ximera.org/differentialequations/main/undeterminedCoeff/undeterminedCoeff}{5.4} and
\href{https://xerxes.ximera.org/differentialequations/main/undeterminedCoeff/undeterminedCoeff2}{5.5}, and is again called
\dfn{method of undetermined coefficients}. Since
the underlying ideas are the same as those in
Trench \href{https://xerxes.ximera.org/differentialequations/main/undeterminedCoeff/undeterminedCoeff}{5.4} and
\href{https://xerxes.ximera.org/differentialequations/main/undeterminedCoeff/undeterminedCoeff2}{5.5},
we will give an informal presentation based on
examples.
...
\end{eqnarray*}
Comparing coefficients of like powers of $x$ on the right sides of this equation and $\eqref{eq:9.3.3}$ shows that $u_p$ satisfies $\eqref{eq:9.3.3}$
if
$$
\begin{array}{rcr}
5D&=&5\\
...
\addplot[domain=0:2.1,smooth, thick, blue] {(1+2*x-x^2+x^3)*exp(x)};
...
\addplot[domain=0:1,smooth, thick, blue] {(1/24)*(x^2)*(-4+4*x+x^2)*exp(2*x)};
...
\addplot[domain=0:4.2,smooth, thick, blue] {-(1/2)*((2-x)*cos(x)+(1+x)*sin(x))*exp(x)};
...
\addplot[domain=0:9.2,smooth, thick, blue] {-(1/2)*x*((3-2*x)*cos(x)+(1+x)*sin(x))*exp(-x)};
