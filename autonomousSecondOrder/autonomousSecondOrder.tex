...
\begin{tikzpicture}[scale=1]
   % y-axis
  \draw[-stealth, thick, black] (0,-3) -- (0,3) node[above] {$v$};
  % x-axis
  \draw[-stealth, thick, black] (0,0) -- (6,0) node[right] {$y$};

  % Circle
  \draw[blue] (3,0) circle(2);

  % Circle
  \draw[blue] (3,0) circle(1);

    % Dashed lines
  \draw[dashed, blue] (3,1) -- (0,1) node[left, black] {$\delta$};
  \draw[dashed, blue] (3,2) -- (0,2) node[left, black] {$\epsilon$};
  \draw[blue] (3,0.2) -- (3,0) node[below, black] {$\overline{y}$};
...
\addplot[smooth, thick, blue] {sin(x+130)};
\addplot[smooth, dashed, blue] {1}
    node[pos=0.9, anchor=south, black] {\small $y = R$};
\addplot[smooth, dashed, blue] {-1}
     node[pos=0.9, anchor=north, black] {\small $y = -R$};
      \end{axis}
        \end{tikzpicture}
\end{center}

\begin{center}
\geogebra{ke6ykten}{800}{600}
\end{center}
...
\begin{center}
\begin{tikzpicture}[scale=1]
  \draw[black] (0,0) circle(0.1);
  \draw[-, gray] (0,-5) -- (0,0);
  \draw[-, line width=2pt, black] (0,0) -- (3,-4);
   \draw[stealth-stealth, dashed, black] (0.4,0.4) -- (3.4,-3.6);
   \node at (2.4, -1.7) {$L$};
  \draw[black, fill=gray] (3,-4) circle(0.4);
  \node at (0.4, -1.4) {$y$};
  \node at (3, -4.8) {$m$};
  \draw[dashed] (0,-1.2) arc[start angle=270,end angle=306.9,radius=1.2];
\end{tikzpicture}
\end{center}
...
\begin{center}
\begin{tikzpicture}[scale=1]
  \draw[-, line width=2pt, black] (-2,-1) -- (-2,1);
  \draw[-, line width=2pt, black] (2,0) -- (2,2);
\draw[black, fill=gray] (2,2) circle(0.2);
\draw[black, fill=gray] (-2,-1) circle(0.2);
   \draw[-, dashed, gray] (0,-2.5) -- (0,2.5);
   \node at (-3, -2.5) {(a) Stable equilibrium};
   \node at (3, -2.5) {(b) Unstable equilibrium};
   \draw[black] (-2,1) circle(0.05);
 \draw[black] (2,0) circle(0.05);
\end{tikzpicture}
\end{center} 
...
%   \draw[-, line width=2pt, black] (0,0) -- (0,-5);
%   \draw[black] (0,0) circle(0.1);
%  \draw[black, fill=gray] (0,-5) circle(0.4);
%  \draw[-, dashed, gray] (0,0) -- (4.3,2.5);
%   \draw[black, fill=white] (4.3, 2.5) circle(0.4);
%    \draw[-, dashed, gray] (0,0) -- (-4.3,2.5);
%   \draw[black, fill=white] (-4.3, 2.5) circle(0.4);
%  \draw [->,dashed, line width=0.5pt,-stealth]  (-2,-5.3) to[out=340, in=200] (2, -5.3);
%   \draw [->,dashed,line width=0.5pt,-stealth]  (5.59,0.92) to[out=100, in=320] (3.59, 4.38);
%   \draw [->,dashed,line width=0.5pt,-stealth]  (-3.59,4.38) to[out=220, in=80] (-5.59, 0.92);
% \end{tikzpicture}
% \end{center}
...
%   \draw[black] (0,0) circle(0.1);
%  \draw[black, fill=gray] (0,-5) circle(0.4);
%  \draw[-, dashed, gray] (0,0) -- (3,-4);
%   \draw[black, fill=white] (3, -4) circle(0.4);
%    \draw[-, dashed, gray] (0,0) -- (-3,-4);
%   \draw[black, fill=white] (-3, -4) circle(0.4);
%   \node at (0.5, -1.5) {$y_{\text{max}}$};
%    \node at (-.6, -1.6) {$-y_{\text{max}}$};
%   \draw[-stealth,dashed] (0,-1.2) arc[start angle=270,end angle=306.9,radius=1.2];
%    \draw[-stealth,dashed] (0,-1.2) arc[start angle=270,end angle=233,radius=1.2];
%  \draw [->,dashed, line width=0.5pt,-stealth]  (-2,-5.3) to[out=340, in=200] (2, -5.3);
%   \draw [->,dashed, line width=0.5pt,-stealth]  (2,-5.8) to[out=200, in=340] (-2, -5.8);
%   \end{tikzpicture}
%   \end{center}
...
It can be shown %(see Exercise~\ref{exer:4.4.21} for a partial proof)
...
The corresponding solutions %,shown below, 
are said to be \dfn{oscillatory}
...
\addplot[smooth, thick, blue] {(cos(10*x)+sin(10*x))*exp(-x)};
      \end{axis}
        \end{tikzpicture}
\end{center}
...
this case.) 
%The following figure shows a direction field and
